...
.NOTES
    ...
    Date-Modified:  2025-06-24 12:27:16

.SYNOPSIS
    Uninstalls ConnectWise RMM Agent and cleans up remnants.

.DESCRIPTION
    This script automates the uninstallation of the ConnectWise RMM Agent.
    It performs the following tasks:

    1. Attempts to uninstall ITSPlatform using its official uninstaller.
    2. Falls back to WMI-based uninstallation for ITSPlatform if the official method fails.
    3. Uninstalls SAAZOD using its dedicated uninstaller.
    4. Stops and force-closes all related processes (platform-agent*, SAAZ*, rthlpdk).
    5. Stops and force-closes all related services (ITSPlatform*, SAAZ*).
    6. Deletes related services from the system.
    7. Deletes all associated folders, including logs and program data.
    8. Removes any rogue "C:\Program" file if found.
    9. Deletes all associated registry keys, including installer remnants.

.EXAMPLE
    .\Uninstall-CWRMMAgent.ps1
#>
...
function Find-RegistryKey {
	...
	foreach ($subKey in $subKeys) {
		...
		Find-RegistryKey -Path $subKey.PSPath
	}
}
...
Get-CimInstance -Class Win32_Product -Filter "Name='ITSPlatform'" | ForEach-Object { $_.Uninstall() }
...
# Start the search
$ProductRegistryKeysToRemove = Find-RegistryKey -Path $startPath
